...
import { API, api, type LifeClock } from "@/lib/api";
...
export default function Dashboard() {
  ...
  // Life clocks: the new engine output, fetched alongside everything the
  // dashboard already loads so the existing panels can show it too.
  const [clocks, setClocks]             = useState<LifeClock[]>([]);
  const clockById = Object.fromEntries(clocks.map(c => [c.shipmentId, c]));
  ...
  const fetchAll = async () => {
    try {
      const [locRes, cmdRes, auditRes] = await Promise.all([
        ...
        fetch(`${API}/api/v1/command-center`),
  ...
  useEffect(() => {
    ...
    socket.on("action.completed", (data: any) => {
      ...
      fetch(`${API}/api/v1/audit`).then(r => r.json()).then(d => setAudit(d.events || [])).catch(() => {});
      fetch(`${API}/api/v1/command-center`).then(r => r.json()).then(d => setKpis(d.kpis)).catch(() => {});
      api.lifeClocks().then(r => setClocks(r.clocks)).catch(() => {});
    });
  ...
  const triggerDisruption = async (type: string, location: string, key: string) => {
    ...
    await fetch(`${API}/api/disruptions`, {
  ...
  const handleReset = async () => {
    ...
    setActiveRecMeta(null);
    setFleetDispatchLine(null);
    await fetch(`${API}/api/v1/reset`, { method: "POST" }).catch(() => {});
  ...
  const approveRec = async (rec: RecommendationData) => {
    try {
      await fetch(`${API}/api/v1/recommendations/${rec._id}/approve`, { method: "POST" });
  ...
  const rejectRec = async (id: string) => {
    await fetch(`${API}/api/v1/recommendations/${id}/reject`, {
      method: "POST", headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen bg-[#060b14] text-slate-100 font-sans">
      ...
      <main className="max-w-[1600px] mx-auto px-5 py-6 space-y-6">
        ...
        {/* ── KPI row ─────────────────────────────────────────────────────────── */}
        {/* Leads on the life clock: how many shipments are running out of usable
            cargo life, and what that is worth. The old row counted alerts;
            this one counts decisions. */}
        <div data-tour="kpis" className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          {[
            {
              label: "Need a decision",
              value: clocks.filter(c => c.state === "red" || c.state === "black").length,
              color: "text-red-400",
              sub: `${clocks.filter(c => c.state === "amber").length} on watch`,
            },
            {
              label: "Value at risk",
              value: `$${Math.round(clocks.reduce((n, c) => n + c.usdAtRisk, 0) / 1000)}k`,
              color: "text-amber-400",
              sub: `${clocks.reduce((n, c) => n + c.dosesAtRisk, 0).toLocaleString()} doses`,
            },
            { label: "Active Disruptions",  value: disruptions.filter(d => d.status === "Active" || !d.status).length, color: "text-amber-400", sub: "" },
        ...
        {/* Simulation time controls — the world runs on a simulated clock, so a
            demo minute can be an hour of shipment time. */}
        <TimeBar />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
          ...
          <div data-tour="map" className="xl:col-span-8 rounded-xl overflow-hidden border border-slate-800/60">
            ...
            <LiveMap
              ...
              lifeClocks={clockById}
              focusShipmentIds={
                // Derived from the recommendations the engines actually
                // produced. This used to be a hardcoded list of SHIP-MVP-10x
                // IDs that no longer exist in the seed, so focus mode silently
                // matched nothing.
                activeScenario
                  ? recommendations.map(r => r.entityId).filter(Boolean)
                  : undefined
              }
            />
          </div>
          ...
          <div data-tour="sensor-feed">
            ...
            <div className="space-y-1 max-h-[280px] overflow-y-auto">
              {logs.length === 0
                ...
                : logs.map((log, i) => {
                    // Defensive: a reading without a timestamp must still get a
                    // unique key, or React collapses the whole feed into one row.
                    const key = `${log.shipmentId}-${log.timestamp ?? log.recordedAt ?? `i${i}`}`;
